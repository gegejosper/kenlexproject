            ...
            <table class="table table-striped table-bordered">
                ...
                @foreach($dataCustomer as $User)
                  <tr class="item{{$User->boxId}}">
                    <td> <a href="/admin/subscribers/{{$User->boxId}}" class="name">{{$User->fname}} {{$User->lname}}</a> </td>
                    <td>{{$User->boxId}}</td> 
                    <td>
                    <?php 
                     if($User->status == 0){
                    ...
                    <td class="" style="width:150px;"><a href="javascript:;" class="activate-modal btn btn-small btn-success" data-id="{{$User->id}}" data-name="{{$User->fname}} {{$User->lname}}"><i class="btn-icon-only icon-pencil"> </i> Activate</a><a href="/admin/subscribers/{{$User->boxId}}" class="btn btn-info btn-small" data-id="{{$User->id}}"><i class="btn-icon-only icon-search"> </i> View</a></td>
                   </tr>
                @endforeach
